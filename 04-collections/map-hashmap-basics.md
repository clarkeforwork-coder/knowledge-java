...
- **`get` 走同一條路**：算 hash → 跳到桶 → 桶裡用 `equals()` 逐一比對。這就是平均 O(1) 的來源，也再次說明 key 的 `hashCode`/`equals` 必須成對正確（見規劃中的〈equals 與 hashCode 契約〉）
- **碰撞（collision）**：不同 key 落在同一桶，桶內串成鏈逐一比對——碰撞越多越接近 O(n)。好的 hashCode 讓 key 均勻散開
- **負載因子與擴容**：預設容量 16、負載因子 0.75——元素數超過「容量 × 0.75」就**擴容一倍並全員重新分桶（rehash）**。至於 hash 怎麼擾動、鏈太長變紅黑樹（樹化）這些內部細節，見規劃中的 🔬〈HashMap 內部原理〉
...
hash 的世界到這裡是「會用且用對」；桶裡的完整風景——hash 擾動、擴容搬遷、鏈變紅黑樹——見規劃中的 🔬〈HashMap 內部原理〉。而撐起這一切的地基「equals 與 hashCode 到底怎麼寫」，就是下一篇：見規劃中的〈equals 與 hashCode 契約〉。
