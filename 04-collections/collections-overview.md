...
這張地圖上每個節點接下來各自展開：List 兩兄弟誰快誰慢、什麼時候真的該用 LinkedList，見規劃中的〈List：ArrayList vs LinkedList〉；hash 家族憑什麼 O(1)、去重到底怎麼判斷「同一個」，見規劃中的〈Map：HashMap 基礎與正確使用〉和〈equals 與 hashCode 契約〉。
