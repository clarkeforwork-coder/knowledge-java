...
| **有序性** | 程式碼被重排 | 編譯器/CPU 為了效能調換執行順序（本篇點到為止，完整理論見 🔬 [Java Memory Model 與 happens-before](deep-jmm-happens-before.md)） |
...
「進出鎖等於同步記憶體」「volatile 寫直達主記憶體」這些白話背後有一套精確的規則——happens-before。想知道 JVM 到底承諾了什麼、重排序的邊界在哪，見 🔬 [Java Memory Model 與 happens-before](deep-jmm-happens-before.md)。而下一個實務問題是：thread 這麼貴（上一篇算過帳），任務又這麼多——見 [ExecutorService 與執行緒池參數](executorservice-and-thread-pools.md)。
