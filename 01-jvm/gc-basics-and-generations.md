...
**每一種 GC 都有 STW，差別只在停多久、停幾次。** 現代收集器（G1、ZGC）的演化方向就是把大停頓拆小、把工作挪到與應用並行的階段去做——各家策略的比較見 🔬 [GC 演算法比較（G1、ZGC）](deep-gc-algorithms.md)。對 🔰 讀者，先建立這個判斷：**Minor GC 的停頓通常無感，讓服務「偶爾卡一下」的多半是 Full GC**——它出現在 log 裡就值得查。
...
3. **回收後的量從 3M 緩慢爬到 6M**——那是 40 個 survivor 逐步累積、晉升的痕跡。這條「地板」就是你服務真正的存活集：**地板平穩 = 健康；地板不停爬升 = memory leak 的形狀**，接下來就該抓 heap dump 了（見 [事故排查工具箱](troubleshooting-toolbox.md)）
...
下一步有兩條線：想知道 G1、ZGC 這些收集器各自怎麼把停頓變短，見 🔬 [GC 演算法比較（G1、ZGC）](deep-gc-algorithms.md)；想知道地板爬升之後怎麼辦——jps、jstack、jmap、heap dump 的事故現場操作，見 [事故排查工具箱](troubleshooting-toolbox.md)。
